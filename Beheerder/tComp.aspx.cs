...
{
	/// <summary>
	/// This control is used to display and edit content of the 'tComp' table.
	/// </summary>
	public class tComp: frmTable
	{
		protected System.Web.UI.WebControls.DataGrid _grid;
		protected System.Web.UI.WebControls.DropDownList ddlSelGridItem;
		private cComps _Comps = null;
		...
		private void BindGrid(int editIndex, bool bSelecting)
		{
			using (DAL_OleDb oDal = new DAL_OleDb())
			{
				_Comps = new cComps(oDal).GetAsList(); 
				_grid.DataSource = _Comps;
				...
				if (!bSelecting)
				{
					...
					ddlSelGridItem.DataSource = _Comps;
					ddlSelGridItem.DataTextField = "Descr";
					ddlSelGridItem.DataValueField = "Comp";
		...
		private void FillRow(DataGridItem dgRow, cComp row, string sCommandName)
		{
			...
			sValue = ((TextBox)dgRow.FindControl(sPrefix + "Descr")).Text;
			row.Descr = sValue;

			sValue = ((TextBox)dgRow.FindControl(sPrefix + "Comment")).Text;
			row.Comment = sValue;
