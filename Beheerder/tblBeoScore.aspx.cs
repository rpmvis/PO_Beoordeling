...
{
	/// <summary>
	/// This control is used to display and edit content of the 'tblBeoScore' table.
	/// </summary>
	public class tblBeoScore: frmTable
	{
		...
		private void BindGrid(int editIndex, bool bSelecting)
		{
			using (DAL_OleDb oDal = new DAL_OleDb())
			{
				...
				_Scors = new cScores(oDal).GetAsList(sPeri); 
				_grid.DataSource = _Scors;
				...
				if (!bSelecting)
				{
					...
					ddlSelGridItem.DataSource = _Scors;
					ddlSelGridItem.DataTextField = "BeoID_BeoNaam_Elem";
		...
		private void FillRow(DataGridItem dgRow, cScor row, string sCommandName)
		{
			...
			sValue = ((TextBox)dgRow.FindControl(sPrefix + "Scor")).Text;
			row.Scor = sValue;

			sValue = ((TextBox)dgRow.FindControl(sPrefix + "Toel")).Text;
			row.Toel = sValue;

			sValue = ((TextBox)dgRow.FindControl(sPrefix + "Doel")).Text;
			row.Doel = sValue;

			sValue = ((TextBox)dgRow.FindControl(sPrefix + "Afsp")).Text;
			row.Afsp = sValue;
		}
		...
		private void _grid_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			try
			{
				...
				using (DAL_OleDb oDal = new DAL_OleDb())
				{
					_Scors = new cScores(oDal).GetAsList();
					iRows = _Scors.Delete(sKeyValue);
				}
				if (iRows == 1)
				{
					BindGrid(-1, false);
