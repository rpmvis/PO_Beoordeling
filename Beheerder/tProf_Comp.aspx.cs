...
{
	/// <summary>
	/// This control is used to display and edit content of the 'tProf_Comps' table.
	/// </summary>
	public class tProf_Comps: frmTable
	{
		protected System.Web.UI.WebControls.DataGrid _grid;
		protected System.Web.UI.WebControls.DropDownList ddlSelGridItem;
		private cProf_Comps _Prof_Comps = null;
		...
		private void BindGrid(int editIndex, bool bSelecting)
		{
			using (DAL_OleDb oDal = new DAL_OleDb())
			{
				_Prof_Comps = new cProf_Comps(oDal).GetAsList(); 
				_grid.DataSource = _Prof_Comps;
				...
				if (!bSelecting)
				{
					...
					ddlSelGridItem.DataSource = _Prof_Comps;
					ddlSelGridItem.DataTextField = "Prof_Comp";
					ddlSelGridItem.DataValueField = "Prof_CompId";
		...
		private void FillRow(DataGridItem dgRow, cProf_Comp row, string sCommandName)
		{
		...
		private void _grid_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			try
			{
				...
				{
					_Prof_Comps = new cProf_Comps(oDal).GetAsList();
					iRows = _Prof_Comps.Delete(sKeyValue);
				}
				if (iRows == 1)
				{
					BindGrid(-1, false);
