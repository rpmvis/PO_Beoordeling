<%@ Page language="c#" AutoEventWireUp="false" Codebehind="behImport XML actX control XMLDOM.aspx.cs" Inherits="PO_Beoordeling.behImport_XML_ActX_XMLDOM" %>
<HTML>
	<HEAD>
		...
		<script language="javascript">
			...
			{
				var sFile = getEl("inpFile").value; 

				// defensive: on file is empty
				if(sFile === "")
				{
					alert("Selecteer eerst een XML bestand (extensie .xml)!");
					LoadXML = false;
					return;
				}
				
				// defensive: on no .xml extension
				var len= sFile.length;
				var sExtension = sFile.substring(len - 4, len);
				
				if (sExtension.toLowerCase() !== ".xml")
				{
					alert("De extensie van het bestand is foutief.\nSelecteer een XML bestand (extensie .xml)!");
					LoadXML = false;
					return;
				}
				

				var sQuestion = "Voordat u importeert is het erg handig als tabel 'tTL_Afde_Peri' (Teamleiders per afdeling en periode) gevuld is voor de import periode.\n\n" +
			...
			function LoadXML(sFile) {
				try
				{
					...
					if (xmlDoc.parseError.errorCode != 0) {
						...
						alert("Java Script error: " + myErr.reason);
					} else {
						root = xmlDoc.documentElement;
						var txtXML = getEl("txtXML");
				...
				{
					sMsg = "Java Script error : " + e.description + "\n\n"
	...
	<body class="body" onload="init();">
		<div id="top_leftalign">
			<table>
				...
				<tr>
					<td class="titelbalk" colSpan="2">Import vanuit XML bestand</td>
				</tr>
			</table>
		</div>
				...
				<TR vAlign="top">
					...
					<TD colSpan="2" rowSpan="2">
						<form id="Form1" method="post" runat="server" enctype="multipart/form-data">
							<TABLE height="121" cellSpacing="0" cellPadding="0" width="755" border="0" ms_2d_layout="TRUE">
								...
								<TR vAlign="top">
									...
									<TD style="HEIGHT: 52px">
										<asp:label id="lblFile" runat="server" Font-Bold="True">XML bestand (*.xml)</asp:label></TD>
									<TD colSpan="2" style="HEIGHT: 52px">
										<!-- file input runs locally! -->
										<input id="inpFile" type="file" size="85" style="WIDTH: 625px; HEIGHT: 22px"></TD>
								</TR>
								<TR vAlign="top">
									<TD colSpan="3" height="25"></TD>
									<TD>
										<INPUT id="btnLoad" style="WIDTH: 200px; HEIGHT: 27px" onclick="Doorgaan_met_Import();" type="button" value="Data laden in database" name="btnLoad">
										<asp:Button id="btnLoad2" runat="server" Text="Data laden in database" Height="0px" Width="0px"></asp:Button></TD>
								</TR>
								<TR>
									<TD colSpan="4">&nbsp;</TD>
								</TR>
							...
							<asp:TextBox id="txtXML" runat="server" Width="736px" Height="120px" TextMode="MultiLine"></asp:TextBox>
						</form>
					</TD>
				</TR>
				<TR vAlign="top">
