...
{
	/// <summary>
	/// This control is used to display and edit content of the 'tFunc' table.
	/// </summary>
	public class tFunc: frmTable
	{
		...
		private void BindGrid(int editIndex, bool bSelecting)
		{
			using (DAL_OleDb oDal = new DAL_OleDb())
			{
				...
				if (!bSelecting)
				{
					...
					ddlSelGridItem.DataSource = _Funcs;
					ddlSelGridItem.DataTextField = "Func";
					ddlSelGridItem.DataValueField = "Func";
				...
				using(DAL_OleDb oDal = new DAL_OleDb())
				{
					...
					switch(e.CommandName)
					{
						...
						case "_edtUpdate":
							...
							string sFuncId = tbx.Text; 
							if (!row.Load(oDal, sFuncId)) row = null; 
							break;
					}
		...
		private void FillRow(DataGridItem dgRow, cFunc row, string sCommandName)
		{
			string sPrefix = "";
			switch (sCommandName)
			{
			...
			string value;
//			value = ((TextBox)dgRow.FindControl(sPrefix + "FuncId")).Text;
//			row.FuncId = System.Convert.ToInt32(value);

			value = ((TextBox)dgRow.FindControl(sPrefix + "Func")).Text;
			row.Func = value;
		}
