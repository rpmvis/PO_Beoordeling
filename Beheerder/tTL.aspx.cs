...
namespace PO_Beoordeling
{
	...
	public class tTL: frmTable
	{
		...
		private void BindGrid(int editIndex, bool bSelecting)
		{
			using (DAL_OleDb oDal = new DAL_OleDb())
			{
				_TLs = new cTLs(oDal).GetAsList(); 
				_grid.DataSource = _TLs;
				...
				if (!bSelecting)
				{
					...
					ddlSelGridItem.DataSource = _TLs;
					ddlSelGridItem.DataTextField = "TLGebruikersnaam_Naam";
					ddlSelGridItem.DataValueField = "TLMsnr";
				...
				using(DAL_OleDb oDal = new DAL_OleDb())
				{
					...
					switch(e.CommandName)
					{
						case "_insUpdate":
							break;
						case "_edtUpdate":
							int iKeyValue = System.Convert.ToInt32(sKeyValue);
							if (!row.Load(oDal, iKeyValue)) row = null; 
							break;
					}
		...
		private void FillRow(DataGridItem dgRow, cTL row, string sCommandName)
		{
			...
			sValue = ((TextBox)dgRow.FindControl(sPrefix + "TLGebruikersNaam")).Text;
			row.TLGebruikersNaam = sValue;

			sValue = ((TextBox)dgRow.FindControl(sPrefix + "TLWachtwoord")).Text;
			row.TLWachtwoord = sValue;
		}
